...
//				randomBot.add("面白い話を聞きたいニャ！");
		...
		slackService.addSlacklet(new Slacklet() {
			...
			public void teee(SlackletRequest req) {
				...
				System.out.println("名前: " + req.getSender().getRealName());
...
//	                            resp.reply(userDisp + "さん。" +  resultHelp);
							...
							try {
								...
								con = DriverManager.getConnection(
										"jdbc:mysql://自身のip:3306/データベース名", "MySQLユーザー名","MySQLパスワード");//データベースに接続
								System.out.println("MySQLに接続できました。");

								Statement stm = con.createStatement();
								String sql = "select * from bot order by rand() lmit 1;";
								ResultSet rs = stm.executeQuery(sql);

								while (rs.next()) {
									String randomName = rs.getString("MOJI");
									String userDisp = req.getUserDisp();
									resp.reply(userDisp + "さん" + randomName);
